...
2. **Terminal:**
   ...
   - Type `flask --app app run --host 0.0.0.0 --port 9000 --no-debugger --no-reload` to run the app.
     - The terminal will say your app is `Running on http://127.0.0.1:9000/`. Click the forwarded port/badge to open it.
...
4. **Rebuild or update your container**
   ...
   **Open browser automatically:** This repo is configured to open your browser automatically when port 9000 is forwarded (see `.devcontainer/devcontainer.json` `"portsAttributes" -> "9000" -> "onAutoForward": "openBrowser"`). You can change this behavior by editing that setting (e.g., to `"notify"`) and rebuilding the container with **Dev Containers: Rebuild Container**.
